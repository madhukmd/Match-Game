...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.gameTime, 1000)
  }
  ...
  filterThumbnails = () => {
    ...
    const filter = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )
    return filter
  }

  thumbnailClick = clickUrl => {
    const {imgUrl} = this.state
    const isTrue = imgUrl === clickUrl
    if (isTrue) {
      const newImgUrl =
  ...
  render() {
    const {imgUrl, activeTabId, score, initialTime, gameRunning} = this.state
        ...
        <div className="match-game-container">
          {gameRunning && (
            <div className="game-container">
              <img src={imgUrl} alt="match" className="display-img" />
          ...
          {!gameRunning && <Scorecard score={score} restart={this.restart} />}
        </div>
      </div>
    )
  }
